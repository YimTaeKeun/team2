...
const Product_list = () => {
    ...
    const names = ["아디다스 티셔츠", "세미 와이드 히든 밴딩 슬랙스", "후드티(회색)", "조거 팬츠", "흰 티", "Yale Dog 후드티", "아디다스 티셔츠", "세미 와이드 히든 밴딩 슬랙스", "후드티(회색)", "조거 팬츠", "흰 티", "Yale Dog 후드티"];
    let product_list = []
    for(let i = 0; i < clothes.length; i++) product_list[i] = {Cloth: clothes[i], Sale: Math.floor(Math.random() * 95) + 5, Name: names[i], Price: Math.floor(Math.random() * 10000) * 10 + 1000};
